...
参考世界级企业体系重构明烛组织架构；用GAIA benchmark真实测试发现并修复能力问题。

#### 企业级组织架构（回答问题2）
- 参考CEO/董事会/C-level/部门体系，重构明烛层级
- `config/organization.yaml`：组织架构配置
  - 董事会 = 用户 + SOUL.md（价值观/红线）
  - CEO = 明烛主人格（战略决策、任务委派、最终裁决）
  - C-level：CTO(震造+巽风)、CSO(艰守)、COO(坤载)、CCO(兑泽)
  - 部门：决策部(乾断)、表达部(离明)、观察部(坎观)
- `agent_system/enterprise_workflow.py`：企业级工作流引擎
  - 5阶段标准流程：立项→规划→执行→审查→复盘
  - 任务板机制：CEO创建任务，部门自主领取
  - Heartbeat：执行中汇报进度和阻塞
  - 去官僚：去掉PPT/会议纪要/审批流，保留高效决策
- 人格层级化：不再是平铺，有明确上下级关系

#### GAIA真实测试（回答问题1）
- 诚实承认：之前test_stress.py是自编测试，不是业内benchmark
- 新增 `tests/test_gaia_dimensions.py`：参考GAIA benchmark维度
  - 多步推理、工具使用、知识检索、数学计算
  - LLM-as-a-Judge评估（不是正则匹配）
  - 测真实能力，不是代码逻辑
- **发现真实问题**：多步推理不完整
  - 问题：分析到一半就停，没给最终结论
  - 根因：max_tokens不足(1500) + prompt没要求最终结论
  - 修复：max_tokens增至2000 + prompt要求"必须给出最终结论"
  - 验证：修复后正确给出"4周"答案，坎观还指出离明初始错误
